...
function AnnouncementsCard({
  ...
  announcements: SiteSettings["announcements"] | Record<string, unknown>;
  onSave: (announcements: Announcements) => void;
  isSaving: boolean;
}) {
  const [editingAnnouncements, setEditingAnnouncements] = useState<Array<{ message: string; type: string }>>(
    (announcements as Announcements).mainBanner || (announcements as Announcements).orderComplete ? 
      [{ message: (announcements as Announcements).mainBanner || '', type: 'mainBanner' },
       { message: (announcements as Announcements).orderComplete || '', type: 'orderComplete' }] : []
  );
...
function PricingCard({
  ...
  const updatePricing = (field: string, value: number | unknown[]) => {
