...
describe("Transaction API", () => {
  test("new tx builder API should work with chaining", async () => {
    ...
    const artworkData = {
      ...
      userId: "user123",
      artwork: {
        originalImageS3Key: "s3://bucket/test.jpg",
        mmPerPixel: 0.5,
      ...
      await ddb.tx(tx => 
        tx
          ...
          .createArtwork({
            ...
            userId: userId,
            artwork: {
              originalImageS3Key: "s3://bucket/welcome-template.jpg",
              mmPerPixel: 1.0,
      ...
      await ddb.tx(tx => 
        tx
          ...
          .createArtwork({
            ...
            userId: userId,
            artwork: {
              originalImageS3Key: "s3://bucket/userart.jpg",
              mmPerPixel: 0.3,
      ...
      await ddb.tx(tx => 
        tx
          ...
          .updateArtwork(artwork => {
            artwork.artwork.mmPerPixel = 0.8;
            artwork.artwork.imageCenterXy = { x: 200, y: 300 };
            return artwork;
          }, "artwork789")
      );
    } catch (error: unknown) {
      ...
      await ddb.tx(tx => 
        tx
          ...
          .updateArtwork(artwork => {
            ...
            artwork.artwork.imageCenterXy = { x: 250, y: 250 };
            artwork.artwork.mmPerPixel = 0.1;
            return artwork;
          }, "artwork456")
